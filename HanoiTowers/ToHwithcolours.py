...
class TowersOfHanoi:
    def __init__(self, root):
        ...
        self.color_button = tk.Button(self.frame, text="Pick Background Color", command=self.pick_color)
    ...
    def start_game(self):
        try:
            ...
            self.disk_colors = [self.random_color() for _ in range(n)]
            self.draw_rods()
            self.moves = 0
    ...
    def draw_rods(self):
        ...
        for rod, x in rod_positions.items():
            self.canvas.create_rectangle(x - rod_width / 2, 50, x + rod_width / 2, rod_height, fill=rod_color)
